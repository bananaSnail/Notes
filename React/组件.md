...
- 源码解析
  - this.setState(newState),newState存入pending队列中，是否处于batch update的，Y保存于组件中dirtyComponents，N遍历所有的dirtyComponents，调用updateComponent，更新pending state or props
...
- 合成事件：在生成fiber节点的时候，与之对应的dom节点还有可能未挂载，onclick这样的事件处理函数作为fiber节点的props。因此react提供了事件注册、事件触发、事件执行机制。
...
https://www.cnblogs.com/WindrunnerMax/p/14438610.html
...
- 运行时优化
  ...
  - react几大版本演变
    ...
    - React 16：主要就是包含 Reconciler 和 Renderer、Scheduler；
      - 优点：提出了一个非常重要的概念，Concurrent Mode；Reconciler 这一层，使用 Fiber 架构进行了重构。Reconciler 这一层，使用 Fiber 架构进行了重构；render 函数产生的结果是 React element，而 Fiber 节点，由 React Element 创建而来。
      - react16更新过程：
      ...
      - 优先级控制：如果一次更新在运行过程中被中断，然后重新开始一次新的更新，我们可以说：后一次更新打断了前一次更新。
        ...
        - 计算过期时间 expirationTime = startTime（当前时间）+ timeout
